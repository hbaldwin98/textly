<script lang="ts">
  ...
  async function handleCreateDocument() {
    if (!newDocumentTitle.trim() || !documentManager) return;

    try {
      const newDoc = await documentManager.createDocument(newDocumentTitle.trim());
  ...
  async function handleRenameDocument() {
    if (!renamingDocumentId || !newTitle.trim() || !documentService) return;
    ...
    try {
      ...
      documentActions.updateDocument(updatedDoc);
      renamingDocumentId = null;
      newTitle = "";
    } catch (err) {
      documentActions.setError(
  ...
  function handleDropdownClick(event: MouseEvent, documentId: string) {
    ...
    openDropdownId = openDropdownId === documentId ? null : documentId;
...
<Modal {isOpen} title="📄 Document Manager" size="lg" on:close={handleClose}>
  <div class="p-6">
    ...
    {:else}
      ...
      <div class="mb-6 space-y-4">
        ...
        <div class="relative">
          ...
          {#if searchTerm}
            ...
            >
              <svg
                class="w-4 h-4 text-gray-400 dark:text-zinc-500 hover:text-gray-600 dark:hover:text-zinc-300 transition-colors"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
        ...
        {#if isCreating}
          <div
            class="p-4 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950/20 dark:to-indigo-950/20 border border-blue-200 dark:border-blue-800 rounded-xl animate-in slide-in-from-top-2 duration-200"
          >
            <div class="flex items-center gap-2 mb-3">
              ...
              <div>
                <h4 class="font-medium text-gray-900 dark:text-zinc-100">
                  Create New Document
                </h4>
        ...
        {:else}
          ...
          >
            ...
            <svg
              class="w-4 h-4 text-gray-400 dark:text-zinc-600 group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-all duration-200 group-hover:translate-x-0.5 ml-4"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
      ...
      {#if errorMessage}
        <div
          class="mb-4 p-4 bg-gradient-to-r from-red-50 to-pink-50 dark:from-red-950/30 dark:to-pink-950/30 border border-red-200 dark:border-red-800 rounded-xl animate-in slide-in-from-top-2 duration-200"
        >
          <div class="flex items-start gap-3">
            <div
              class="w-8 h-8 rounded-lg bg-red-100 dark:bg-red-900/50 flex items-center justify-center flex-shrink-0"
            >
              <svg
      ...
      {#if loading}
        <div class="flex flex-col items-center justify-center py-12">
          <div class="relative">
            <div
              class="w-12 h-12 rounded-full border-4 border-blue-100 dark:border-blue-900/30"
            ></div>
            <div
              class="w-12 h-12 rounded-full border-4 border-blue-600 dark:border-blue-400 border-t-transparent animate-spin absolute top-0 left-0"
            ></div>
          </div>
          <p class="mt-4 text-gray-600 dark:text-zinc-400 font-medium">
            Loading documents...
          </p>
      ...
      {:else if filteredDocuments.length === 0}
        ...
        <div class="text-center py-12">
          {#if searchTerm}
            <div class="space-y-4">
              <div
                class="w-16 h-16 mx-auto rounded-full bg-gray-100 dark:bg-zinc-800 flex items-center justify-center"
              >
                <svg
              ...
              <div>
                <p class="text-gray-700 dark:text-zinc-300 font-medium">
                  No documents found
                </p>
          ...
          {:else}
            <div class="space-y-6">
              <div
                class="w-20 h-20 mx-auto rounded-full bg-gradient-to-br from-blue-100 to-indigo-100 dark:from-blue-950/50 dark:to-indigo-950/50 flex items-center justify-center"
              >
                <svg
              ...
              <div>
                ...
                <p class="text-gray-600 dark:text-zinc-400 mt-2">
                  Create your first document to start writing and organizing
                  your thoughts.
                </p>
              </div>
              <button
      ...
      {:else}
        <!-- Documents List -->
        <div
          class="space-y-3 max-h-96 overflow-y-auto custom-scrollbar relative"
        >
                    ...
                    <div class="flex items-start justify-between">
                      <div class="flex-1 min-w-0 pr-3">
                        <div class="flex items-center gap-2 mb-1">
                          ...
                          <h3
                            class="font-semibold text-gray-900 dark:text-zinc-100 truncate group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors duration-200"
                          >
                        ...
                        {#if document.content}
                          <p
                            class="text-sm text-gray-600 dark:text-zinc-400 mt-1.5 line-clamp-2 leading-relaxed"
                          >
                            {truncateContent(document.content, 120)}
                          </p>
                        {:else}
                          <p
                            class="text-sm italic text-gray-400 dark:text-zinc-500 mt-1.5"
                          >
                            Empty document
                          </p>
                        {/if}
                        <div class="flex items-center gap-3 mt-3">
                          ...
                          <div
                            class="w-2 h-2 rounded-full bg-green-400 dark:bg-green-500"
                            title="Synced"
                          ></div>
                        </div>
                      </div>
                      <div class="flex items-center gap-2">
                        <svg
                          class="w-5 h-5 text-gray-300 dark:text-zinc-600 group-hover:text-blue-500 dark:group-hover:text-blue-400 flex-shrink-0 transition-all duration-200 group-hover:translate-x-1"
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
                        ...
                        <button
                          onclick={(e) => {
                            e.stopPropagation();
                            handleDropdownClick(e, document.id);
                          }}
...
{#if openDropdownId && dropdownTrigger}
  ...
  >
    <div class="py-2">
      <button
        onclick={() =>
          startRename(documentList.find((d) => d.id === openDropdownId)!)}
      ...
      >
        <div
          class="w-8 h-8 rounded-lg bg-blue-100 dark:bg-blue-950/50 flex items-center justify-center group-hover/item:bg-blue-200 dark:group-hover/item:bg-blue-900/50 transition-colors duration-200"
        >
          <svg
            class="w-4 h-4 text-blue-600 dark:text-blue-400"
            fill="none"
      ...
      <button
        onclick={() => handleDeleteDocument(openDropdownId!)}
      ...
      >
        <div
          class="w-8 h-8 rounded-lg bg-red-100 dark:bg-red-950/50 flex items-center justify-center group-hover/item:bg-red-200 dark:group-hover/item:bg-red-900/50 transition-colors duration-200"
        >
          <svg
            class="w-4 h-4 text-red-600 dark:text-red-400"
            fill="none"
