...
<div class="relative" bind:this={dropdownElement}>
  ...
  {#if isOpen && !modelState.isLoading}
    <div class="absolute bottom-full left-0 right-0 mb-2 bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 rounded-lg shadow-lg z-50 p-3">
      {#if modelState.availableModels.length === 0}
        <div class="text-center text-gray-500 dark:text-zinc-400 py-4">
          No models available
        </div>
      {:else}
        <div class="text-xs font-medium text-gray-600 dark:text-zinc-400 mb-3 uppercase tracking-wide">
          Select AI Model
        </div>
        
        <div class="grid grid-cols-2 gap-2">
          {#each modelState.availableModels as model}
            <button
              class="p-3 text-left rounded-lg border transition-all hover:shadow-md"
            ...
            >
              <div class="flex items-center gap-2 mb-2">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="text-xs text-gray-500 dark:text-zinc-500 capitalize">
                    {model.provider}
                  </div>
                </div>
                {#if modelState.selectedModel?.id === model.id}
              ...
              <div class="flex flex-wrap gap-1">
                {#each Object.entries(model.capabilities) as [capability, enabled]}
                  {#if enabled}
                    {@const capabilityKey = capability as keyof ModelCapabilities}
                    <span class="inline-flex items-center text-xs bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-zinc-400 px-1.5 py-0.5 rounded">
                      {modelService.getCapabilityIcon(capabilityKey)}
                    </span>
                  {/if}
                {/each}
              </div>
  ...
  <div class="flex items-center gap-2">
    ...
    <button
      class="flex items-center gap-2 px-3 py-2 text-sm bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 rounded-lg hover:bg-gray-50 dark:hover:bg-zinc-800 transition-colors flex-1"
      onclick={togglePane}
    ...
    >
      {#if modelState.isLoading}
        ...
        <span class="text-gray-500 dark:text-zinc-400">Loading...</span>
      {:else if modelState.selectedModel}
        <span class="text-lg">{modelState.selectedModel.icon}</span>
      ...
      {:else}
        <span class="text-gray-500 dark:text-zinc-400">Select Model</span>
      {/if}
    ...
    {#if modelState.selectedModel && selectedModelCapabilities}
      <div class="flex gap-1">
        <!-- Reasoning Toggle -->
        <button
          class="p-2 rounded border transition-colors {selectedModelCapabilities.reasoning 
        ...
        <!-- Internet Toggle -->
        <button
          class="p-2 rounded border transition-colors {selectedModelCapabilities.internet 
